...
export default class StartUp {
    ...
    protected _isImportantIndent(): boolean {
        const indentBase = this.getConfig().get("indentBase", "firstline") as string;
        return indentBase == "dontchange";
    }

    protected _getSimilarNeighbourInSingleLine(sel: Selection) {
        return this.narrow(0, this.editor.document.lineCount - 1, sel.active.line, this._isImportantIndent());
    }

    protected _downRangeToAlign(sel: Selection, ranges: LineRange[]): LineRange[] {
        let res: LineRange;
        let start = sel.start.line;
        let end = sel.end.line;
        while (true) {
            res = this.narrow(start, end, start, this._isImportantIndent());
    ...
    protected _getRanges(): LineRange[] {
        var ranges: LineRange[] = [];
        this.editor.selections.forEach(sel => {
            if (sel.isSingleLine)
                return ranges.push(this._getSimilarNeighbourInSingleLine(sel));
            ranges = this._downRangeToAlign(sel, ranges);
        });
        return ranges;
    }
    ...
    protected _apply() {
        var ranges: LineRange[] = this._getRanges();
        this.editor.edit((editBuilder) => {
            for (let i = 0; i < ranges.length; ++i) {
                ...
                editBuilder.replace(location, this._formatter(ranges)[i].join("\n"));
            }
        });
    }

    protected _formatter(ranges: LineRange[]) {
